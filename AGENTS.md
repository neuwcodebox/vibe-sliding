...
- Export an experimental editable PowerPoint deck with `npm run export:pptx:editable`.

Use npm, not pnpm or yarn.

## Key Dependencies

- Runtime: React 19, Vite, Tailwind CSS 4, Framer Motion, Lucide React, Mermaid, Recharts, clsx, tailwind-merge, PptxGenJS, and dom-to-pptx.
...
- For PPTX export changes, run the affected `npm run export:pptx` or `npm run export:pptx:editable` command when a dev server is available.
