...
Presenter View는 청중 화면을 비워 둔 채 현재·다음 슬라이드, 대본, 경과 시간,
슬라이드 이동 기능을 제공합니다.
...
검정/흰색 전환, 레이저, 펜, 되돌리기, 주석 지우기를 제공하며 `B`, `W`, `R`, `D`, `Z`, `C`
단축키도 동일하게 사용할 수 있습니다. 이 화면에서는 `F`로 전체 화면을 전환하며,
Presenter View에는 청중 화면 연결 상태가 표시됩니다.
