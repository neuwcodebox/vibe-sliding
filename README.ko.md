...
- 대본·슬라이드 이동·타이머·청중 제어를 제공하는 별도 Presenter View
...
Presenter View는 청중 화면을 비워 둔 채 현재·다음 슬라이드, 선택적 대본,
타이머 조작, 슬라이드 목록·점프, 대본 글자 크기 조절을 제공합니다.

발표 도구 버튼에는 단축키가 툴팁으로 표시됩니다.

- `B` / `W`: 청중 화면을 검정 / 흰색으로 전환
- `F`: 청중 화면을 고정한 채 발표자만 슬라이드 이동
- `R`: 레이저 포인터 켜기·끄기. 현재 화면 미리보기 위에서 움직이면 양쪽 화면에 표시
- `D`: 펜 주석 켜기·끄기. 현재 화면 미리보기 위를 드래그하면 양쪽 화면에 표시
- `C`: 현재 슬라이드의 펜 주석 지우기

펜 주석은 임시 표시이지만 발표를 열어 둔 동안 해당 슬라이드에 유지됩니다. 발표
스테이지에서는 텍스트 선택과 브라우저 기본 드래그를 허용하지 않습니다.

모니터가 하나라면 청중 화면 중앙 하단에 붙은 작은 반투명 펼치기 버튼을 사용하세요.
청중 화면에서 검정/흰색 전환, 레이저, 펜, 주석 지우기 메뉴를 직접 열 수 있으며,
`B`, `W`, `R`, `D`, `C` 단축키도 동일하게 사용할 수 있습니다. 타이머와 청중 화면
고정은 별도의 발표자 화면이 있어야 의미가 있으므로 Presenter View에만 제공합니다.
...
export const slides = [
  // existing slides
  {
    ...
    // 선택적 Presenter View 대본입니다. 청중 화면에는 표시되지 않습니다.
    notes: ['결론부터 소개합니다.', '여기에서 질문을 받습니다.'],
  },
]
```

`notes`는 선택 사항입니다. 등록하면 각 문자열이 Presenter View 대본 패널의 별도
문단으로 표시되며 청중 화면에는 나타나지 않습니다. 사용자가 이 패턴을 외울 필요는
없지만, Agent가 어떤 작업을 해야 하는지 이해하는 데 도움이 됩니다.
